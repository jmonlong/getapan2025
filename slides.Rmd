...
date: "06/11/2025 - https://github.com/jmonlong/getapan2025"
output:
...
![](imgs/simple_nested_snarl.summary.masked.svg)
...
![](imgs/loopy_chain.summary.masked.svg)
...
1. in-house scripts that compares REF path vs ALT(s) paths (e.g. Pangenie).
...
![](imgs/cartoon-giraffe-trotting.gif){width=10%}
...
![](imgs/haplotype-sampling1.svg){width=90%}


## Haplotype sampling for mapping

Simplify graphs by sampling haplotypes similar to reads 
![](imgs/haplotype-sampling2.svg){width=90%}

# Giraffe results on HPRC v2 Minigraph-cactus

Pangenome with 462 haplotypes + 2 reference genomes

![](imgs/giraffe_runtime_memory.svg){width=70%}
\ 
\ 
\ 
\ 
![](imgs/giraffe-pushes-tortoise.gif){width=20%}

# Giraffe results on HPRC v2 Minigraph-cactus

Small variant calling with DeepVariant, structural variant calling with Sniffles2 vs genotyping with `vg call`

![](imgs/giraffe-calling.svg){width=80%}
...
:::: {style="display: grid; align-items: start; justify-items: start; grid-template-columns: 1fr 2fr; grid-column-gap: 10px;"}
...
![](imgs/lab_photo_costumes_2022.jpg)
...
![](imgs/snarl-algo-paten2017-2.png)

# Duplications in zip code trees

![](imgs/zipcode_snarl_duplication.svg){width=100%}

# Inversions in zip code trees

![](imgs/zipcode_snarl_inversion.svg){width=100%}
